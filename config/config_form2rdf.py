...
class Form2RDFController:
    ...
    def add_entries_for_property(self, subject, predicate, obj, root_id=None):
        ...
        while True:
            ...
            node_kind_selection = self.get_node_kind_selection(
                permitted_node_kind, entry_id)
    ...
    def get_node_kind_selection(self, permitted_node_kind, entry_id):
        ...
        if permitted_node_kind not in ['BlankNodeOrIRI', 'BlankNodeOrLiteral', 'IRIOrLiteral']:
            raise ValueError('Not valid nodeKind option: ' +
                             permitted_node_kind)
        ...
        if node_kind_selection not in node_kind_options:
            raise ValueError(
                'Not valid nodeKind selection: ' + node_kind_selection)
    ...
    def add_literal_entry(self, subject, predicate, obj, entry_id):
        ...
        if datatype == XSD.boolean:
            ...
            if entry:
                self.rdf_result.add(
                    (subject, predicate, Literal(True, datatype=XSD.boolean)))
                return True
            # Entry with prefix 'Unchecked ' -> False
            elif self.form_input.get('Unchecked ' + entry_id):
                self.rdf_result.add(
                    (subject, predicate, Literal(False, datatype=XSD.boolean)))
                return True
            # Neither -> No value
            else:
                return False
            ...
            self.rdf_result.add(
                (subject, predicate, Literal(entry, datatype=datatype)))
            return True
        return False
    ...
    def add_iri_entry(self, subject, predicate, entry_id):
        ...
        if entry:
            self.rdf_result.add(
                (subject, predicate, URIRef(self.validate_iri(entry))))
            return True
        else:
            return False
    ...
    def add_blank_node_entry(self, subject, predicate, obj, entry_id):
        ...
        for p in included_properties:
            ...
            found_entry_for_property = self.add_entries_for_property(
                node, p[0], p[1], nested_property_id)
